...
6. **Website hook** public API is a union on `status` (`idle|pending|ready|error`): `ready` carries `token` **and** `url`, every other status carries neither — a partial payload must degrade to `pending`, never reach `Room.connect`. Gate fetch with **`can.enterLive` first**, then `meeting.status === "STARTED"`, then route ids (`memberId` / `memberToken` / `meetingId`). Quiet network retry uses `skipNetworkToast` (declared in `website/src/types/extends/global.ts`, honored in `axios.ts`) — do not cast Axios config. Sole consumer is `useMeetingLiveKitRoom`.
