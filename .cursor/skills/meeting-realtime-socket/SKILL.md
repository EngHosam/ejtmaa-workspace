...
6c. **Live agenda seed** (first empty `live_state` only): nested per-id `Y.Map` under `agendaItems`; map SQL `sort_order` → `sortOrder`, `subject`, **`status` from SQL**; seed `isLiveCreated: false`; set `currentAgendaItemId` **after** `agendaItems` (`null`). Session cancel is `status: "CANCELED"` — never remove the map key / never add `isDeleted`. (`meeting-live-state.md` §1.2 / §9; `agenda-item-domain.md`).
