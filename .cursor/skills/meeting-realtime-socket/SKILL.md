...
6c. **Live agenda seed** (first empty `live_state` only): nested per-id `Y.Map` under `agendaItems`; map SQL `sort_order` → `sortOrder`; seed `status: "WAITING"`, `isLiveCreated: false`; set `currentAgendaItemId` **after** `agendaItems` (`null`). Session cancel is `status: "CANCELED"` — never remove the map key / never add `isDeleted`. Session-only fields are not SQL columns (`meeting-live-state.md` §1.2 / §9d; `agenda-item-domain.md`).
...
6e. **Live decisions seed** (first empty `live_state` only): nested per-id `Y.Map` under `decisions` from **all** SQL phases; map SQL fields; seed `isLiveCreated: false`; live status union includes `CANCELED`; per-decision `votes` = **empty** nested `Y.Map` (logical `{}` — no member slots, no SQL Vote seed); set `currentDecisionId` **after** `decisions` (`null`). Do not store plain `{}` as the Yjs `votes` value. Writers not shipped (`meeting-live-state.md` §1.2 / §9f; `decision-domain.md`; `vote-domain.md`).
...
12. **Live map mirror:** if `MeetingLiveMap` / participant / agenda / talk / decision / vote fields / `MEETING_LIVE_*` change, update **both** `backend/src/app/types/meeting.ts` and `website/src/types/meeting.ts` identically in the same change; confirm with a file diff. Seed nested `participants`, `agendaItems`, and `decisions` (with empty nested `votes` `Y.Map`) as per-id `Y.Map`s in `MeetingLiveDocHelper` (not plain objects). Live-mutation flag is named `isLiveCreated` (not bare `isCreated` / `isUpdated` / `isLiveUpdated`). Talk queue: `talkTurn` on participant + `currentTalkMemberId` on root (after `participants`). Decisions: `currentDecisionId` after `decisions`; live status may be `CANCELED`; do not pre-book vote keys per participant.
