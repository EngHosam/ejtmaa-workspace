...
- Customer: `me` (+ nested `currentSubscription`), `notifications`, `organization`, `members`, `member(id)`, `messageTemplates`, `messageTemplate(id)`, `meetings`, `meeting(id)` (+ nested `participants`, `agendaItems`, `decisions`, `talkRecords`), `plans`, `plan(id)`, `subscriptions`, `subscription(id)`
