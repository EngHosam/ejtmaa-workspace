...
description: >-
  ...
  MeetingTalkQueuePage chair floor + FIFO queue admin, MeetingPrimaryButton, MeetingMetaChip,
  ...
  MeetingPageStub under meeting/pages,
  MeetingLiveBroadcast, MeetingPageOverlay, MeetingPrimaryButton, MeetingMetaChip,
  useMeetingPage, useMeetingAgenda, useMeetingTalkQueue, useOrganization,
...
6. Put `disabled` / `livePulse` / `ariaLabel` on tile **defs** before the list. Drawer translator only (`meetingLayout.drawer`) — keys `liveLockedAria`, `liveBroadcastingAria`, `agendaDiscussingAria`, `talkQueueActiveAria`. Do not import `init` / `agenda` / `talkQueue` translators into the drawer.
...
8. New product surfaces: replace the title stub in the matching `meeting/pages/Meeting*Page.tsx` (keep the file); do not invent a second route. Shared chrome-only placeholder: `MeetingPageStub`. `"attendance"`, `"live"`, `"agenda"`, and `"talkQueue"` are already product UI — extend those pages, do not re-stub them.
...
11. Copy under `ui.layouts.meetingLayout` — `drawer`, `header`, `linking`, `init`, `live`, `broadcast`, `overlay`, `attendance`, `agenda`, and `talkQueue` (ar + en identical keys). Page uses **one** translator namespace for its body. Init = first person; attendance body = third person. Enum status/type strings must match backend `general.ts` labels.
