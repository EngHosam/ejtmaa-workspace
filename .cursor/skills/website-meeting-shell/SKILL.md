...
11. `MeetingInitPage`: org logo/name + live meta; when `can.attend` show CTA `actions.attend` **and** caption `attendRequiresForRoom` under the button; remaining-duration from session `attendWindow` (never call `useMeetingAttendWindow` here); first-person attended title + relative duration; `roomUnlockedHint` = `textTertiary`.
