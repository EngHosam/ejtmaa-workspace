...
description: >-
  ...
  tiles (chair vs member/viewer), full-row live tile, in-shell MeetingPage
  (MeetingPageProvider + Meeting.tsx page switch), and header request-to-speak
  for non-chair. Use when editing MeetingDrawerPanel, MeetingHeader,
  MeetingInitPage under meeting/pages, useMeetingPage, Meeting.tsx, or
  MeetingLayout READY chrome. For linking/can/actions/meeting/me use
...
5. Keep `live` full-row (`wide` → `gridColumn: 1 / -1`).
...
9. Colors from `OrganizationColors` / `defaultOrganizationColors()`. Style props: `buttonReset` in `baseCssStyle`; behavioral CSS in `cssStyle`.
10. Copy under `ui.layouts.meetingLayout.drawer` and `.header` (ar + en, identical keys).
