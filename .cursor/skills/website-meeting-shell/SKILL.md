...
description: >-
  ...
  MeetingInitPage lobby, MeetingLivePage (waiting/start/broadcast placeholder),
  ...
  under meeting/pages, MeetingLiveBroadcast, MeetingPrimaryButton, MeetingMetaChip,
  useMeetingPage, useOrganization, HomeMark color overrides for meeting wells,
...
- Shipping or changing Meeting room (`MeetingLivePage`, `MeetingLiveBroadcast`, start CTA).
- Shipping or changing the chair attendance log (`MeetingAttendancePage`, `useMeetingAttendance`, `MeetingAttendanceCard`).
- Changing shared org primary CTA (`MeetingPrimaryButton`).
...
- `.cursor/rules/website-backend-enum-label-mirror.mdc` (status/type display labels)
...
10. Copy under `ui.layouts.meetingLayout` — `drawer`, `header`, `linking`, `init`, `live`, and `attendance` (incl. `title`; ar + en identical keys). Page uses **one** translator namespace for its body. Init = first person; attendance body = third person. Enum status/type strings must match backend `general.ts` labels.
...
12. Drawer `live`: disable when `!can.enterLive`; `MeetingLivePage` bounce to `"init"` when locked. After enterLive: `STARTED` → `MeetingLiveBroadcast`; else waiting + `MeetingPrimaryButton` when `can.startMeeting` → `actions.startMeeting`. Do not extend start side effects in the page.
