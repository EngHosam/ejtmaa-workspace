...
4. Form screen: create-only stable identify + `removeOnExit: false`; `submittingRef`; fields subject / type (`FormChoiceField`) / datetime (`FormDateTimeField` with `minDate={meetingDatetimeMinDate()}` + `minDateError`) / notify_start_at (`FormDateTimeField`, no `minDate`, gap stated in `subTitle`) / min_members / chairperson (`FormEntityPickerField` `ident="members"`); `afterSentSuccess` → details with `replace: true`; no manual success toast. `MeetingBasicsModal` carries the same two datetime fields — change both together or the edit path drifts from create.
4b. Schedule rules (10-minute lead, 5-minute invite gap, draft-only editing, voting-only quorum) are backend-owned. Read them from `meetings/meetingScheduleLead.ts`; never inline the numbers or re-filter participant types per screen. Any note you render must match the server condition — `.cursor/rules/website-backend-policy-mirror.mdc`, `.cursor/rules/meeting-lifecycle-approve-lock.mdc`.
