...
- `Plan` is non-actor platform catalog SKU (الباقة; not org-owned). Do not name this model `Package` (language-sensitive). BIGINT PK; MultiLang `name` / optional `description`; `price` DECIMAL(18,2) SAR; `billing_period` (`planBillingPeriod`: `MONTHLY`|`YEARLY`) + `billing_period_count` (default 1); optional `max_members` / `max_meetings_per_month` (null = unlimited); `sort_order`; `status` (`planStatus`: `ACTIVE`|`DISABLED`, default `ACTIVE`). No `code` SKU field. No payment/subscription state on this model. Seed helper arrays must still not be named `plan`/`plans` (see demo-seed conventions).
