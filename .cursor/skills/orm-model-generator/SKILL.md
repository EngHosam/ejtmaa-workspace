...
- `MessageChannel` is non-actor under Organization (`organization_id` real FK). Type enum `messageChannelType`: `CUSTOM_EMAIL` | `ADWHATS` | `ADWHATS_PRO`. Status enum `messageChannelStatus`: `ACTIVE` | `DISABLED` (default `ACTIVE`; create-time connectivity test may leave it active; send/token failures can disable). Channel-specific nullable columns (SMTP vs Ad Whats credentials). Not used for platform email (`EJTMAA_EMAIL` templates have no channel row).
- `MessageTemplate` is non-actor under Organization (`organization_id` real FK). Kind enum `messageTemplateType`: `EJTMAA_EMAIL` | `CUSTOM_EMAIL` | `ADWHATS` | `ADWHATS_PRO`. Optional `message_channel_id` (null only for `EJTMAA_EMAIL`). Content columns `subject` / `body` / `variables` (JSONB map) are nullable at ORM; write-path enforces by kind. `belongsTo(MessageChannel)` only — no inverse `hasMany` on `MessageChannel` in current slice. Contract: `docs/platforms/backend/contracts/message-template-domain.md`; rule: `.cursor/rules/message-template-domain.mdc`.
...
- `Subscription` is non-actor, owned by **Customer** (`customer_id` real FK — not Organization). Also `plan_id` → Plan. Snapshot fields: `plan_price`, `plan_billing_period` (`planBillingPeriod`), `plan_billing_period_count`, optional `plan_max_members` / `plan_max_meetings_per_month`. Lifecycle columns: `starts_at`, `ends_at`, `status` (`subscriptionStatus`: `ACTIVE`|`EXPIRED`|`REPLACED`, default `ACTIVE`). No `PENDING`/`CANCELED`. Attrs groups: `//relations`, `//info`. `Customer.hasMany(Subscription)`; `Customer.hasOne(Subscription.scope("current"), { as: "currentSubscription" })` where scope `current` is a **function** (`ACTIVE` + `ends_at > now`); `Plan.hasMany(Subscription)`. Current catalog Plan is `currentSubscription.plan`. Hourly `ExpireSubscriptionsTask` promotes overdue `ACTIVE` rows to `EXPIRED`. Entitlement create: static `Subscription.subscribe(op)` (calendar window, prior `ACTIVE` → `REPLACED`, new `ACTIVE`). Callers: payment finalize (and future writers). No MyFatoorah fields on this model. Contract: `docs/platforms/backend/contracts/subscription-domain.md`; rule: `.cursor/rules/subscription-domain.mdc`.
- `MyFatoorahInvoice` is non-actor gateway payment session owned by **Customer** (`customer_id` real FK). Columns: `private_key`, `amount` (SAR charged for the session), `related_data` (JSONB soft intent), `status` (`myFatoorahInvoiceStatus`: `PENDING`|`COMPLETED`|`FAILED`|`CANCELED`, default `PENDING`), `mf_invoice_id` / `mf_method_id` / `mf_payment_url` / `mf_payment_id`. `related_data` is a **discriminated union** on `pay_for` — arms hold type-specific soft ids only (subscription arm: `plan_id`, `plan_billing_period`, optional `subscription_id`); do **not** put shared money fields inside arms. No ORM FK to Subscription/Plan. `Customer.hasMany(MyFatoorahInvoice)`. Unique indexes on `private_key` and `mf_invoice_id`. **No** GQL invoice roots/bridges. Contract: `docs/platforms/backend/contracts/myfatoorah-invoice-payment-domain.md`; rule: `.cursor/rules/myfatoorah-invoice-payment-domain.mdc`.
