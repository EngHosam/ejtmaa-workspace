...
9. **Stage scroll:** one `customScroll` owner wraps featured chair + remote grid; featured scrolls with the roster (never a non-scrolling strip above the grid). Pad the scroll content so the floating bar does not cover the last row.
10. **Floating controls:** `Absolute` over the stage (`z={2}`) + compact `cardBackground` / `shd` cluster (**no** page-background strip, **no** border on the cluster). Keep `MeetingPageOverlay` at a higher `z`. Mute-all well chrome stays nested inside the cluster.
11. **Copy:** add both `ar` and `en` under `ui.layouts.meetingLayout.broadcast` with identical key sets. A toggle label states the current state; the accessible name is the control noun; a mute-all name says "everyone else" (LiveKit excludes the sender).
12. **Identity comes from the session roster** (`meeting.participants`), with the LiveKit peer name as fallback only. LiveKit is a media plane, not an identity or attendance source.
13. **Live-map state on the stage** (talk queue today): signal it with tile chrome and an action-row control — never by swapping the featured chair tile, and never by changing publish/mute state. Tile state badges are `role="img"` with a label; decorative rails are `aria-hidden`. If you order peers by that state, use the same total comparator (ordering key + stable id tie-break) as the page that owns the state. Contract: `.cursor/rules/meeting-talk-queue.mdc`, skill `website-meeting-talk-queue`.
14. **Document ceilings honestly.** If the change leaves a gap (no enforcement, no retry, no recovery), add it to `flow-meeting-broadcast.md` §10 and the `organization-host-routing.md` §8 limit list rather than implying it works.
15. **Verify with existing scripts only:** `yarn type-check` in `website/` (and `backend/` when the controller changed). Then the two-browser pass in `flow-meeting-broadcast.md` §13 — including the OS-level mic-mute case before suspecting the code.
