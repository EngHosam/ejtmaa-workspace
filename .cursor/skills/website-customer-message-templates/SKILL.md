...
7. Form kinds: `EJTMAA_EMAIL` (subject+body); `CUSTOM_EMAIL`/`ADWHATS`/`ADWHATS_PRO` (+ channel); Pro requires `meta_template` picker (`adwhatsProApprovedTemplates`, gated on channel id) + `FormMessageTemplateVariablesField`. `FormChoiceField` `name="lang"` (`AR`/`EN`); create default `AR`; Pro `readOnly` from picker `language`. Do not use `LanguageSwitch` or `FormTextField` for language. Insert chips via `headerArea` (human labels). Placeholders config: `messageTemplatePlaceholders.ts`. Type `readOnly` on update. Clear channel when create type changes; clear `meta_template` when channel changes.
