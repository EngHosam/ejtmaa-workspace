...
- **Client phase** (`boot.client`, shared by both host modes):
  ...
  - prepares socket via `socket.prepareSocket(myInstance)` — on an organization host returns without a socket (Meeting opens `/meeting` from `MeetingLiveProvider` in `MeetingLayout`); on apex, namespace `customer` for an authed customer, no socket otherwise,
