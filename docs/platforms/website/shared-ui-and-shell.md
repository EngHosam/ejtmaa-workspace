...
- **Empty / LaneFailed** (`Wrong.tsx`): absolute overlays for ResultLane; empty uses `FiLayers` (not Masdaria-style `FiInbox`). Same empty glyph on `DataTable`. i18n `ui.components.wrong.*`. Both accept optional copy overrides (`Empty`: `title` / `sub_title`; `LaneFailed`: `title` / `description`) that fall back to the shared keys when blank — a surface with domain-specific failure copy passes them instead of forking the chrome (meeting broadcast + Meeting linking FAILED do; `flow-meeting-broadcast.md` §8). `onRetry` renders the retry button only when a caller actually has a retry path. Because both are absolute fills, the parent must be positioned (`relative`). Guest / Able remain planned/target where not present. Only use Feather icons that exist in the installed `react-icons/fi` export set (e.g. `FiFolderOpen` is absent in current package — causes a blank icon).
...
- **MicrobandCredit** (`src/app/ui/components/MicrobandCredit.tsx`): linked footer credit pill used by shared `Footer.tsx`, `CustomerFooter`, and `LandingFooter`. External link target: `https://microband.com.sa`. Logo asset: `public/images/microband.png`. Copy keys: `ui.components.microbandCredit.lead` and `ui.components.microbandCredit.brand`. The displayed brand wordmark MUST stay **`MicroBand`** (capital `B`) in both `ar.ts` and `en.ts`; the component/file key names remain lowercase `microband*` for historical stability. The dot/wordmark blue `#096fb1` is a third-party brand color exempt from W43 (W57).
