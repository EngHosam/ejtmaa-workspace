...
- The public hook reads `liveStore[MEETING_LIVE_MAP]` (typed `Partial<MeetingLiveMap>`), including `participants` / `currentTalkMemberId` / `agendaItems` / `decisions` once the server document has them.
...
| End meeting | Owned by colocated `InitEndMeetingSection` in the same file (page-only). Renders only when `can.endMeeting` (chair + `STARTED`). `MeetingPrimaryButton` → `await confirm(t("endMeetingConfirm"), "danger")` (`ConfirmModal` helper) → `actions.endMeeting()` on confirm. No end-meeting CTA on broadcast or Meeting room waiting UI. Keys: `endMeeting` / `endMeetingAria` / `endMeetingConfirm` under `meetingLayout.init` (ar + en) |
...
| `src/app/ui/components/meeting/pages/MeetingInitPage.tsx` | `"init"` lobby; colocated `InitAttendSection` + `InitEndMeetingSection` (confirm → `actions.endMeeting`); meta via `MeetingMetaChip`; `pv` only; `bg="@transparent"` | §5.5 |
...
| `src/app/ui/components/meeting/MeetingPrimaryButton.tsx` | org primary CTA (`label` / `ariaLabel` / `onClick`); used by Init attend + Init end + Live start | §5.5 |
...
| `src/resources/translations/ar.ts`, `src/resources/translations/en.ts` | `ui.layouts.meetingLayout` (`header`, `footer`, `linking`, `drawer` incl. live/agenda/talk-queue/decisions aria keys, `init` incl. pre-start ballot branch + `endMeeting` / `endMeetingAria` / `endMeetingConfirm`, `attendance`, `agenda`, `decisions`, `live`, `broadcast`, `overlay.closeAria`) | §5.3, §5.4, §5.5 |
...
| `src/app/ui/components/meeting/hooks/useMeetingLive.tsx` | transport: SyncedStore + `/meeting` sync/update/error + `emitLiveStart` / `emitLiveComplete` + `batch` + `socketRef`; `MeetingLiveProvider` / public `useMeetingLive`; `connect_error` linking branch | §5.1, §5.3 |
...
| `docs/platforms/backend/contracts/meeting-live-state.md` | `minMembersCount` on live map + seed | live-state §1.2 |
...
|---|---|---|
| `src/app/types/meeting.ts` | modified — `MeetingLiveMap.minMembersCount` | §5.1; `meeting-live-state.md` §1.2 |
| `src/app/helpers/MeetingLiveDocHelper.ts` | modified — seed `minMembersCount` on first empty create only (no BLOB backfill) | `meeting-live-state.md` §1.3 |
...
| `docs/platforms/backend/contracts/meeting-live-state.md` | `minMembersCount` seed contract | live-state §1.2 |
...
## 10n) Change set inventory (durable agenda/decision enums + live SQL seed)
...
## 10s) Change set inventory (Init end-meeting CTA)

Chair irreversible end on Meeting info (`init`) only: `InitEndMeetingSection` → `confirm(..., "danger")` → `actions.endMeeting`. No broadcast end CTA. Session/transport lifecycle already in §10r.

### `website/`

| Path | State | Documented in |
|---|---|---|
| `src/app/ui/components/meeting/pages/MeetingInitPage.tsx` | modified — colocated `InitEndMeetingSection` | §5.5 |
| `src/resources/translations/ar.ts` | modified — `init.endMeeting*` | §5.5 |
| `src/resources/translations/en.ts` | modified — `init.endMeeting*` | §5.5 |
| `lib/tsconfig.tsbuildinfo` | generated cache | **excluded** |
...
| Path | State | Documented in |
|---|---|---|
| `docs/platforms/website/organization-host-routing.md` | this §10s + §5.5 Init end row | — |
| `.cursor/rules/website-meeting-shell.mdc` | Init end + confirm | governance |
| `.cursor/rules/website-meeting-live-session.mdc` | end UI surface | governance |
| `.cursor/skills/website-meeting-shell/SKILL.md` | Init end checklist | skill |
| `.cursor/skills/website-meeting-live-session/SKILL.md` | end confirm on init | skill |
...
- `docs/invariants/website.md` W58, W62
