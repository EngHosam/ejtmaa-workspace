...
| `src/types/gql/definitions/customer.graphql` | Customer SDL (`_Me`, `_Organization`, `_Member`, `_MessageTemplate`, `_Meeting`, `_MeetingParticipant`, `Query { me, notifications, organization, members, member, messageTemplates, messageTemplate, meetings, meeting }`; nested `_Meeting.participants`) | `graphql-mirror-and-tooling.md` §2; `repository-inventory.md` § GQL mirrors |
