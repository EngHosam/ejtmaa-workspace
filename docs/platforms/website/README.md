...
| Broadcast A/V client (`useMeetingLiveKitRoom` + `MeetingLiveBroadcast`, camera/mic/sound, cooperative mute-all, raise-hand control + tile queue badges, media ceilings) | `flow-meeting-broadcast.md`; `organization-host-routing.md` §5.5, §8, §10m |
...
| Known limits (`org_host` on LiveKit token; remaining drawer stub `decisionsAndVote`; client-only chair gates incl. agenda status and talk-queue administration; client-allocated `talkTurn`; LiveKit token also gated by `can.enterLive`) | `organization-host-routing.md` §8 |
