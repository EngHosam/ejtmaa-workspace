...
| `DRAFT` only | subscription line: `scheduleNoteSubscriptionRequired` when `me.currentSubscription` exists, else `scheduleNoteSubscriptionMissing` | — |
...
**Subscription is not a readiness row.** Active-subscription guidance for approve lives in the draft **schedule notes** only (`scheduleNoteSubscriptionRequired` / `scheduleNoteSubscriptionMissing` from `me.currentSubscription`). Do not add a readiness checklist item or place denial copy under the Approve button. Backend enforcement: `Customer.can` `MEETING`/`approve` → `getCurrentSubscription` → `MEETING_ACTIVE_SUBSCRIPTION_REQUIRED` (`meeting-domain.md` §9.1b). Live entry (handshake / LiveKit `org_host`) uses the same helper under §9.1c — not this checklist. Checkout UX: `flow-customer-subscription.md`.
...
- Draft create/update remains allowed without a subscription; **approve** (§9.1b) and live entry (§9.1c) require an active subscription.
