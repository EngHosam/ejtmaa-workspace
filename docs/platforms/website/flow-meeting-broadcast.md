...
**Known drift:** `meetingLayout.live.broadcastPlaceholder` (ar + en) is now unreferenced — it belonged to the probe UI this change set replaced. It is left in place pending a cleanup decision; nothing reads it.
...
| `src/resources/translations/ar.ts` | modified — `broadcast.*`, `linking.failedHint`, `failedMessage` punctuation | §9 |
