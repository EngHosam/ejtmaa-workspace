...
- GQL mirrors: `base` + `customer` (`me`, `notifications`, `organization`, `members`, `member`, `messageTemplates`, `messageTemplate`, `meetings`, `meeting` with nested `participants` / `_MeetingParticipant` and `agendaItems` / `_AgendaItem`, `me.canDeleteNotifications`)
