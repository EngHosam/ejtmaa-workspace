...
| `src/app/ui/components/LandingHeader.tsx` | A | § Header / Drawer / Footer |
...
| `src/app/ui/components/LandingHeader.tsx` | `baseCssStyle`→`cssStyle`/shorthands; `bg="@transparent"`; `key={navKey}`; `scrollToTop` aria | § Header; W29/W49 |
