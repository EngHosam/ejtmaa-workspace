...
Customer mirrors currently include organization, member, message-template, meeting, meeting-participant, agenda-item, decision, vote, and talk-record reads from backend contracts (`organization-domain.md`, `member-domain.md`, `message-template-domain.md`, `meeting-domain.md`, `meeting-participant-domain.md`, `agenda-item-domain.md`, `decision-domain.md`, `vote-domain.md`, `talk-record-domain.md`): roots `organization`, `members`, `member(id)`, `messageTemplates`, `messageTemplate(id)`, `meetings`, `meeting(id)`; types `_Organization` / `_Member` / `_MessageTemplate` / `_Meeting` / `_MeetingParticipant` / `_AgendaItem` / `_Decision` / `_Vote` / `_TalkRecord` (nested `_Meeting.participants` + `_MeetingParticipant.member`, `_Meeting.agendaItems`, `_Meeting.decisions`, `_Decision.votes` + `_Vote.member`, `_Meeting.talkRecords` + `_TalkRecord.member`; no root participant/agenda/decision/vote/talkRecord queries).
