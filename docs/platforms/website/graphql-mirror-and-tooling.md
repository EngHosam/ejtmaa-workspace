...
Customer mirrors currently include organization, member, message-template, meeting, meeting-participant, agenda-item, decision, vote, talk-record, plan catalog, and subscription reads from backend contracts (`organization-domain.md`, `member-domain.md`, `message-template-domain.md`, `meeting-domain.md`, `meeting-participant-domain.md`, `agenda-item-domain.md`, `decision-domain.md`, `vote-domain.md`, `talk-record-domain.md`, `plan-domain.md`, `subscription-domain.md`): roots `organization`, `members`, `member(id)`, `messageTemplates`, `messageTemplate(id)`, `meetings`, `meeting(id)`, `plans`, `plan(id)`, `subscriptions`, `subscription(id)`; types `_Organization` / `_Member` / `_MessageTemplate` / `_Meeting` / `_MeetingParticipant` / `_AgendaItem` / `_Decision` / `_Vote` / `_TalkRecord` / `_Plan` / `_Subscription` (nested `_Me.currentSubscription`, `_Subscription.plan`, `_Meeting.participants` + `_MeetingParticipant.member`, `_Meeting.agendaItems`, `_Meeting.decisions`, `_Decision.votes` + `_Vote.member`, `_Meeting.talkRecords` + `_TalkRecord.member`; no root participant/agenda/decision/vote/talkRecord queries; plan roots are public ACTIVE catalog; subscriptions are me-scoped).
