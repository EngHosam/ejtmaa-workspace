...
- `src/types/gql/definitions/customer.graphql` — customer SDL (`_Me`, `_Organization`, `_Member`, `_MessageTemplate`, `_Meeting`, `_MeetingParticipant`, `_AgendaItem`, `_Decision` + root `Query { me, notifications, organization, members, member, messageTemplates, messageTemplate, meetings, meeting }`; nested `_Meeting.participants`, `_Meeting.agendaItems`, `_Meeting.decisions`)
