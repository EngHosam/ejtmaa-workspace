...
**Shipped shared components:** `Header`, `Drawer`, `Footer`, `Logo`, `DrawerMenuIcon`, `HeaderIconButton`, `HomeMark`, `SarMark`, `SarAmount`, `Breadcrumb`, `useBreadcrumbs`, `LandingHeader`, `LandingFooter`, `LandingMobileDrawer`, `ThemeModeSwitch`, `LanguageSwitch`, `Loadable`, `Toast`, `DataTable`, `ResultLane`, `CardSkeleton`, `LoadMoreButton`, `SearchField`, `SectionHeading`, `FilterOptionChip`, `FilterOptionChips`, `Wrong` (`Empty` / `LaneFailed`), auth (`AuthPageShell`, `AuthTextField`, `AuthNavLink`, `AuthSecondaryNavButton`), form (`FormTextField`, `FormActionButton`, `FormInputWrapper`, `FormChoiceField`, `FormEntityPickerField`, `FormDateTimeField`, …), modals (`FormModal` chrome, `EntityPickerModal`, `DateTimePickerModal`, `ConfirmModal`, `SelectableEntityCard`, `entity-picker/configs/*`), landing home sections (`home/*`), customer shell (`CustomerHeader`, `CustomerFooter`, `CustomerDrawer`, `CustomerSubHeader`, `IdentityAvatar`, `hooks/useMe`, `hooks/useCustomerMembers`, `hooks/useCustomerMeetings`, `hooks/useCustomerMessageChannels`, `hooks/useCustomerSubscriptionPlans`, `hooks/useCustomerSubscriptionPaymentMethods`, `hooks/useCustomerSubscriptionCanSubscribe`, `members/*`, `meetings/*`, `modals/*` registered form modals, `message-channels/*`, `message-templates/*`, `subscription/*`).

**Planned (target contract, not yet in scaffold):** `CustomerBottomBar` / `BottomIcons`, remaining `pages/customer/*` workspace screens (settings, notifications, static info, support), Google social auth UI (`SelectableCard`, `Checkbox` on Register).
...
| Customer form modals | `customer/modals/*` (`MeetingBasicsModal`, `MeetingParticipantAddModal`, `MeetingSubjectModal`, `MeetingTemplatesModal` — registered; see `flow-form-foundation.md` §3.8b) |
...
| Meeting shell (organization host) | `meeting/MeetingHeader`, `meeting/MeetingHeaderMe`, `meeting/MeetingFooter`, `meeting/MeetingDrawerPanel`, `meeting/MeetingDrawerOverlay`, `meeting/MeetingLiveBroadcast`, `meeting/MeetingPageOverlay` (solid floating sheet while `STARTED`; `ph=page.padY`), `meeting/MeetingPrimaryButton`, `meeting/MeetingMetaChip`, `meeting/MeetingAttendanceCard` (own chrome; not HeaderMe), `meeting/MeetingAgendaCard`, `meeting/MeetingTalkQueueCard`, `meeting/MeetingDecisionCard`, `meeting/hooks/useMeetingAttendance` (data VM; no navigation), `meeting/hooks/useMeetingAgenda`, `meeting/hooks/useMeetingTalkQueue` (queue rows + floor VM), `meeting/hooks/useMeetingDecisions` (ballot rows + per-row gates VM), `meeting/hooks/useOrganization`, `meeting/hooks/useMeetingPage` (`MeetingPageProvider`), `meeting/pages/*` (`MeetingInitPage` lobby reads session `attendWindow` and the pending pre-start ballot flag; `MeetingLivePage` waiting/start only; `MeetingAttendancePage` chair log + page-owned bounce; `MeetingAgendaPage`; `MeetingTalkQueuePage` chair queue admin + page-owned bounce; `MeetingDecisionsAndVotePage` ballots for every role; shared `MeetingPageStub` remains for future drawer ids) — `organization-host-routing.md` §5.4, §5.5 (READY shell only; linking gate precedes it); IA: `.cursor/rules/website-meeting-shell.mdc` |
...
Customer workspace: shipped `CustomerHome` command map, members directory + form, meetings directory + create form + details, organization settings, message channels directory, message templates directory, subscription catalog + checkout + paymentReturn. Planned: notifications, static info, support, bottom bar.
