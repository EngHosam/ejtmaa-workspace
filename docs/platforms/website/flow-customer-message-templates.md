...
| `ADWHATS_PRO` | picker required | `meta_template` picker (`adwhatsProApprovedTemplates`, gated on channel id); variables table after a template is selected |
...
- Create: stable `formIdentify` + `d.reset()` on success; changing type clears `messageChannel`; changing channel clears `meta_template`.
- `read` must not echo `messageTemplate` id; hydrate `type` / `messageChannel` / `meta_template` as SelectOption (`requester-read-select-hydrate.md`).
...
| Pro meta picker | `entity-picker/configs/adwhatsProApprovedTemplates.tsx` |
...
| Mode | Behavior |
|---|---|
| create | Stable `formIdentify` (`customer-message-template-form-create`); **`d.reset()` on create success**; type change clears `messageChannel`; channel change clears `meta_template` |
...
| `ui.pages.customer.messageTemplateForm.*` | form (incl. `var*`, `metaTemplate*`, `variables*`) |
| `ui.layouts.customerMainLayout.drawer.itemMessageTemplates` | drawer |

## 6) Locked UI agreements
...
| Pro requires approved-template picker after a channel is selected | `meta_template` → `meta_template_id` + `meta_template_label` |
...
| `src/app/ui/components/customer/message-templates/CustomerMessageTemplateFormScreen.tsx` | Kind branching; inner `AdwhatsProMetaTemplate` | §3–§6 |
...
| `src/app/ui/components/modals/entity-picker/configs/adwhatsProApprovedTemplates.tsx` | Pro approved-template picker; reuses `CustomerMessageTemplateCard` + `selected` | §1 |
| `src/app/ui/components/modals/entity-picker/configs/index.ts` | Register picker idents | §2 |
...
Shared form chrome (`FormInputWrapper`, `FormChoiceField`, `FormEntityPickerField`, `EntityPickerModal`): `flow-form-foundation.md`. Backend: `message-template-domain.md` §9.
