...
| [`decision-domain.md`](contracts/decision-domain.md) | Decision ORM + nested GQL; live `decisions` / `currentDecisionId` / empty nested `votes` |
...
| [`meeting-live-state.md`](contracts/meeting-live-state.md) | Meeting CRDT document, `live_state` BLOB, registry; live agenda + talk-queue + decisions/votes seeds |
...
| [`requester-read-select-hydrate.md`](patterns/requester-read-select-hydrate.md) | `read` SelectOption hydrate (`toEnumForSelect` / `forSelect`) |
