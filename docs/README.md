...
- `docs/platforms/backend/contracts/meeting-live-state.md` — Meeting CRDT document, `live_state` BLOB, registry; live agenda + talk-queue + decisions/votes seeds (`agendaItems` / `talkTurn` / `currentTalkMemberId` / `decisions` / nested `votes` from SQL).
...
- `docs/platforms/cpanel/customer-management.md` — Customers directory.
- `docs/platforms/cpanel/supervisor-home.md` — Supervisor home dashboard.
...
- `docs/platforms/website/flow-meeting-broadcast.md` — Meeting LiveKit A/V client (room hook, controls, ceilings).
