...
Identification sent to the backend (`organizationId` HTTP header on axios; optional `organizationId` on the Meeting handshake in `meeting-socket.ts`) is added only when a value exists — never as an `undefined` placeholder. Shared boot `configs/socket.ts` does **not** send `organizationId`. Organization hosts open no boot socket; Meeting owns `/meeting` via `LiveMeetingProvider` in `MeetingLayout` (`useLiveMeeting` is the public context reader).
