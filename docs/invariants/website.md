...
## W62. Meeting Brand Pair Is Paint-Remapped

The organization-host meeting shell must not paint stored `primary_color` / `secondary_color` (or `BrandColors.navy` / `BrandColors.orange`) as meeting seeds. After `resolveSeed`, each hex goes through `website/src/app/helpers/ColorHelpers.ts`: keep hue; `softenMeetingPrimary` pins saturation/lightness to `#2F5D50`; `softenMeetingSecondary` pins them to `#8FA99A`. The calibration pair paints as itself. `defaultOrganizationColors()` applies the same helpers to the navy/orange fallbacks. The remap is **not** persisted — requester / `org/start` / the customer color fields keep the raw hex.

Do not copy the remap into a meeting component. Do not add a second caller until a second paint surface exists. Do not “fix” the form by writing the remapped hex back. The organization form explains meeting paint on `FormFieldGroup` (`colorsSubtitle`) and still shows the stored picker value.

See `docs/platforms/website/organization-host-routing.md` §5.4, §8.15, `flow-customer-organization.md` §4.1, and `.cursor/rules/website-meeting-secondary-soften.mdc`.
